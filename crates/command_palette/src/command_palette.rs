...
impl PickerDelegate for CommandPaletteDelegate {
    ...
    fn placeholder_text(&self, _window: &mut Window, _cx: &mut App) -> Arc<str> {
        translate("Execute a command...", &*_cx).into()
    }
    ...
    fn render_footer(
        ...
        let keybinding_buttons = if keybind.has_binding(window) {
            Button::new("change", translate("Change Keybinding\u{2026}", cx))
                .key_binding(
            ...
            Button::new("add", translate("Add Keybinding\u{2026}", cx))
                .key_binding(
                    KeyBinding::for_action_in(&menu::SecondaryConfirm, focus_handle, cx)
                        .map(|kb| kb.size(rems_from_px(12.))),
                )
        ...
        Some(
            h_flex()
                ...
                .child(
                    Button::new("run-action", translate("Run", cx))
                        .key_binding(
